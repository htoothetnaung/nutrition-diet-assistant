...
class AssemblyNutritionAssistant:
    def __init__(self, model_path: str, assembly_key: str, nutrition_kb_path: str = "models/accurate_nutrition_kb.json"):
        ...
        if aai is None:
            raise ImportError("assemblyai package not installed. Run: py -m pip install assemblyai")
        ...
        self.gym_keywords = [
            "gym", "workout", "exercise", "training", "routine", "program", "strength",
            "cardio", "running", "walking", "push", "pull", "legs", "rest day", "warm up",
            "cool down", "stretch", "mobility", "reps", "sets"
        ]

        self.health_keywords = [
            "health", "wellness", "hydration", "sleep", "steps", "heart rate", "bmi",
            "balanced diet", "portion", "lifestyle", "recovery"
        ]
        ...
        self.blocked_topics = [
            ...
            "personal", "relationship", "family", "friends", "medical",
    ...
    def is_plan_request(self, text: str) -> bool:
        t = text.lower()
        plan_tokens = [
            "diet plan", "meal plan", "workout plan", "gym plan", "weekly plan",
    ...
    def is_nutrition_query(self, text: str) -> bool:
        ...
        has_health_keyword = any(keyword in text_lower for keyword in self.health_keywords)
        question_patterns = [
        ...
        return has_nutrition_keyword or has_gym_keyword or has_health_keyword or has_question_pattern
    ...
    def generate_kb_answer(self, question: str, food_data: Optional[Dict]) -> Optional[str]:
        ...
        for key in nutrient_map.keys():
            if key in q:
                selected = key
                break
        if selected is None and any(k in q for k in ["nutrition", "nutrients", "macros", "values"]):
    ...
    def lemur_ask(self, question: str, food_data: Optional[Dict] = None) -> str:
        ...
        try:
            ...
            return (task.response or "").strip() or "I'm sorry, I couldn't generate a response."
        except Exception as e:
    ...
    def lemur_plan(self, question: str) -> str:
        try:
            ...
            return (task.response or "").strip() or "I could not create a plan right now."
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="AssemblyAI-Powered Nutrition Voice Assistant")
    parser.add_argument("--model", type=str, required=True, help="Path to Vosk model folder")
    parser.add_argument("--assembly-key", type=str, required=False, default=None, help="AssemblyAI API key (or set ASSEMBLYAI_API_KEY env var)")
    parser.add_argument("--nutrition-kb", type=str, default="models/accurate_nutrition_kb.json", help="Path to nutrition knowledge base")
    ...
    try:
        ...
        if not assembly_key:
            print("Error: AssemblyAI API key is required. Pass --assembly-key or set ASSEMBLYAI_API_KEY environment variable.")
            return

        print("Initializing AssemblyAI-Powered Nutrition Voice Assistant...")
        assistant = AssemblyNutritionAssistant(args.model, assembly_key, args.nutrition_kb)
        ...
        try:
            ...
            try:
                ...
                for i, d in enumerate(devices):
                    ...
                    if d.get('max_input_channels', 0) > 0 and any(k in name for k in preferred_keywords):
                        candidate_idx = i
                        break
                ...
                if candidate_idx is not None:
                    print(f"Auto-selected input device {candidate_idx}: {devices[candidate_idx]['name']}")
            ...
            while True:
                try:
                    ...
                    if assistant.stt.accept_waveform(data):
                        ...
                        if ("stop ryan" in lower_text or "ryan stop" in lower_text or "stop" in lower_text) and assistant.tts.is_speaking():
                            assistant.tts.stop()
                            continue
                        ...
                        try:
                            ...
                            while assistant.tts.is_speaking() and (time.time() - wait_start) < 8.0:
                                time.sleep(0.05)
                        except Exception as e:
                            print(f"❌ TTS Error: {e}")
                        finally:
