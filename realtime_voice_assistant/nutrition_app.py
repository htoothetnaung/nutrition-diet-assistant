...
# Import nutrition knowledge base
sys.path.append(os.path.join(os.path.dirname(__file__), 'training'))
from nutrition_trainer import NutritionKnowledgeBase, NutritionItem

class NutritionVoiceAssistant:
    """Enhanced voice assistant with nutrition knowledge"""
    ...
    def __init__(self, model_path: str, nutrition_kb_path: str = "models/nutrition_kb.json"):
        ...
        self.nutrition_kb = self.load_nutrition_kb(nutrition_kb_path)
    ...
    def load_nutrition_kb(self, kb_path: str) -> Optional[NutritionKnowledgeBase]:
        """Load nutrition knowledge base"""
        try:
            if os.path.exists(kb_path):
                kb = NutritionKnowledgeBase(kb_path)
                print(f"✅ Loaded nutrition knowledge base with {len(kb.nutrition_data)} items")
                return kb
            else:
                print(f"⚠️  Nutrition knowledge base not found at {kb_path}")
                print("   Run 'python training/nutrition_trainer.py --create-sample' to create a sample dataset")
                return None
    ...
    def generate_nutrition_reply(self, text: str) -> str:
        ...
        # Search for the food
        results = self.nutrition_kb.search_food(food_name)
        ...
        if score < 0.3:  # Low confidence
            return f"I'm not sure about '{food_name}'. Did you mean '{best_match.name}'?"
        
        # Generate nutrition response
        response = f"Here's the nutrition information for {best_match.name}:\n"
        response += f"• Calories: {best_match.calories} per 100g\n"
        response += f"• Protein: {best_match.protein}g\n"
        response += f"• Carbohydrates: {best_match.carbs}g\n"
        response += f"• Fat: {best_match.fat}g\n"
        
        if best_match.fiber > 0:
            response += f"• Fiber: {best_match.fiber}g\n"
        if best_match.sugar > 0:
            response += f"• Sugar: {best_match.sugar}g\n"
...
def main():
    parser = argparse.ArgumentParser(description="Nutrition Voice Assistant")
    ...
    try:
        ...
        print("Initializing Nutrition Voice Assistant...")
        assistant = NutritionVoiceAssistant(args.model, args.nutrition_kb)
    ...
    print("🎤 Nutrition Assistant ready! Ask me about food nutrition facts!")
